...
if executable('tidy')
  ...
  autocmd vimrcFileTypeHTML BufWinEnter <buffer> let &l:formatprg = 'tidy -xml -quiet ' .
...
elseif executable('html-beautify')
  ...
  autocmd vimrcFileTypeHTML BufWinEnter <buffer> let &l:formatprg = 'html-beautify -s ' . &l:shiftwidth . ' -f -'
elseif executable('prettier')
  autocmd vimrcFileTypeHTML BufWinEnter <buffer> let &l:formatprg = 'prettier --stdin-filepath=%:S --parser=html --single-quote --tab-width=' . &l:shiftwidth . (&expandtab ? '' : '--use-tabs') . ' --'
endif
