...
if b:formatprg ==# 'prettier' || empty(b:formatprg) && executable('prettier')
  ...
  let b:formatprg_cmd = 'prettier ' . get(b:, 'formatprg_args', '--log-level=error --no-color --no-error-on-unmatched-pattern --single-quote --parser=' . &filetype . ' --vue-indent-script-and-style') . ' '
...
let b:undo_ftplugin = (exists('b:undo_ftplugin') ? b:undo_ftplugin . ' | ' : '') .
      \ 'setlocal formatprg< formatexpr< | unlet! b:formatprg b:formatprg_cmd b:formatprg_args b:prettier_config | silent! autocmd! formatprgsVue * <buffer>'
