...
if b:formatprg ==# 'xmllint' || empty(b:formatprg) && executable('xmllint')
  autocmd BufWinEnter <buffer> ++once exe let &l:formatprg = has('win32')
        \ ? 'cmd /c "set XMLLINT_INDENT=' . shiftwidth() . '&& xmllint --format --recover -"' 
        \ : 'XMLLINT_INDENT=' . shiftwidth() . ' xmllint --format --recover -'
