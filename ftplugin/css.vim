...
elseif b:formatprg ==# 'biome' || empty(b:formatprg) && executable('biome')
  ...
  autocmd formatprgsCss BufWinEnter <buffer> ++once let &l:formatprg = b:formatprg_cmd . ' ' .
...
if exists('#formatprgsCss#BufWinEnter#<buffer>')
  if bufwinnr(bufnr()) != -1
    doautocmd <nomodeline> formatprgsCss BufWinEnter <buffer>
  endif
endif
